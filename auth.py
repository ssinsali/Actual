...
def _github_auth_cfg() -> tuple[str, str, str]:
    ...
    if not token or token in _PLACEHOLDER_TOKENS or "실제" in token:
        raise RuntimeError(
            ...
            "GitHub에서 Generate 한 뒤 복사한 실제 토큰을 넣으세요."
        )
    try:
        token.encode("ascii")
        ...
        raise RuntimeError(
            ...
            "GitHub에서 발급한 실제 토큰만 넣으세요."
        ) from e
    if not _TOKEN_RE.match(token):
        raise RuntimeError(
            "Secrets [github].token 형식이 올바르지 않습니다. "
            "github_pat_... 또는 ghp_... 로 시작해야 합니다."
        )
...
def _github_request(
    ...
    req.add_header("User-Agent", "performance-stats-auth")
    ...
    try:
        ...
        if e.code == 403:
            raise RuntimeError(
                "GitHub 권한 부족(403). 토큰에 저장소 Contents 쓰기 권한이 필요합니다."
            ) from e
        if e.code == 404:
            raise RuntimeError(
                "GitHub 리소스를 찾을 수 없습니다(404). "
                "Secrets의 repo·path·branch 를 확인하세요."
            ) from e
...
def is_admin() -> bool:
    try:
        ...
        data = _load_users_local()
    user = data["users"].get(current_user() or "")
...
def logout_session() -> None:
    ...
    for key in list(st.session_state.keys()):
        if str(key).endswith("confirm_shutdown"):
            st.session_state.pop(key, None)
...
def render_auth_gate() -> bool:
    ...
    with tab_signup:
        ...
        with st.form("signup_form", clear_on_submit=False):
            ...
            signed = st.form_submit_button("회원가입", type="primary", use_container_width=True)
        if signed:
            if new_pw != new_pw2:
